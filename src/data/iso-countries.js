// Reverse map of ISO 3166-1 alpha-3 → alpha-2, so we can feed Intl.DisplayNames
// (which only accepts alpha-2). Source: ISO 3166 / Wikipedia.
...
/** Resolve an ISO-3 code to a human-readable English country name. Falls back
 * to the iso3 code itself if the platform lacks Intl.DisplayNames or the code
 * is unknown. */
export function countryName(iso3) {
  const alpha2 = ISO3_TO_2[iso3]
  if (!alpha2) return iso3
  const dn = displayNames()
  if (!dn) return iso3
  try {
    return dn.of(alpha2) ?? iso3
  } catch {
    return iso3
  }
}
